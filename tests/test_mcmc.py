...
gen_model = SimulatedGLMM(n_grp=200, n_per=10)
...
for i in range(n_chains):
    samples[i] = model.sample_slice_gibbs2(n_samples)
    
az_dict = to_arviz_dict(samples, dict(beta=np.arange(4), 
                                     theta=np.arange(4, 8)))

az_data = az.from_dict(az_dict)
summary = az.summary(az_data)

az.plot_trace(az_data, var_names=['theta'])
